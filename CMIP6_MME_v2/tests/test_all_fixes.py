...
class TestSeasonal:
    ...
    def test_dry_season_hydrological_year(self, minimal_df, wet, dry):
        ...
        jan_apr_days  = 31 + 28 + 31 + 30   # Jan–Apr (2001, not a leap year)
    ...
    def test_completeness_threshold(self, minimal_df, wet, dry):
        ...
        from src.rainfall.seasonal import _wide_to_yearly
        df = minimal_df.copy()
        # Zero out 35% of year-2001 rows (set to NaN) — below 80% threshold
...
class TestMetrics:
    ...
    def test_delta_kge_not_percentage_of_negative_baseline(self):
        ...
        Uses a non-degenerate Raw MME that has negative KGE:
        ...
        assert abs(delta_kge) <= 2.0,      "ΔKGE bounded by KGE range difference"
...
class TestIO:
    def test_parse_csv_accepts_uppercase_scenario(self):
        ...
        assert r["scenario"] == "ssp245", (
            f"scenario must be normalised to lowercase, got {r['scenario']}"
        )
...
# ===========================================================================
# Bug #7 — figures/make.py: np.concatenate guard + single computation
# ===========================================================================

class TestFigureHelpers:
    def test_collect_panel_data_empty_meta_returns_empty_arrays(self):
        """Bug #7: _collect_panel_data must not crash when no stations match meta."""
        from src.figures.make import _collect_panel_data
        obs = pd.DataFrame({
        ...
        result = _collect_panel_data(obs, bc, meta, "Annual", ["ssp245"])
...
# ===========================================================================
# Bug #8 — final_run.py: QC covers PNG + TIFF
# ===========================================================================
...
class TestFreeCorner:
    def test_north_quadrant_is_image_top(self):
        ...
        = NW in geographic space.  North label should land in the upper (north)
        portion of the map bounds.
        """
...
class TestImports:
    ...
    def test_no_stub_map_panel(self):
        """_map_panel stub (dead code) must not be present in make.py."""
        make_src = Path(__file__).parent.parent / "src" / "figures" / "make.py"
        text = make_src.read_text()
